...
def fetch(url: str, timeout: float = 20.0) -> Fetched:
    """One anonymous request to the share host.

    A phone user agent on purpose: the share host renders the page
    for one, and a desktop agent is more often answered with an
    app-download wall.
    """
    return _fetch(
        url,
