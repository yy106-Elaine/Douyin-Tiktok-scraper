    ...
    with SessionLocal() as session:
        ...
        fetch_videos.run(
            ...
            fetcher=lambda url: Fetched(
                url=url, html=_video_page("7687820515369510629"), http_status=200
            ),
        )
    ...
    with SessionLocal() as session:
        for video_id in ("7687820515369510629", "7687818847273685937"):
            fetch_videos.run(
                ...
                [video_id],
                pause_seconds=0,
                fetcher=lambda url, v=video_id: Fetched(
                    url=url, html=_video_page(v), http_status=200
                ),
            )
        ...
        report = fetch_authors.run(
            ...
            [_SEC_UID],
            pause_seconds=0,
            fetcher=lambda url: visits.append(url)
            or Fetched(url=url, html=profile, http_status=200),
        )
