...
女同桌) turned out to cost only 2 of its 12 results, which the filter catches,
and at 33% in scope it outperformed 女同性恋 by ten times.
...
Watch the per-keyword numbers rather than the total — but read them before
pruning. Measured on the first 150 results, 拉拉 yielded 57% in scope and
女同性恋 3%, because 女同性恋 translates cleanly to "lesbian" and the search
maps it across languages while community slang stays Chinese. Nothing was
dropped on that basis: rows are marked rather than deleted and quota is not the
constraint, so precision is cheap while lost recall is permanent.
